...
- **All timestamp columns are `_epoch_ms` (milliseconds).** V43 (Phase 14-09b, 2026-05-18) renamed the last 24 legacy `_epoch` columns across 16 tables to `_epoch_ms` and scaled their values by 1000. New tables MUST use `_epoch_ms` only — never add an `_epoch` column. Read column values as ms directly; pass to `new Date(ms)` without scaling. Historical audit: `context/measurements/2026-05-18-legacy-epoch-columns-audit.md` (now resolved). The pre-V43 dual-convention scar was the source of the 2026-05-17 LSS-debug failure (agent divided seconds-as-seconds by 1000 and got 1970) — V43 closes that vector.
