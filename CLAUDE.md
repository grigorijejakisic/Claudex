...
<!-- critical: drift-risk=availability, domains=angel,hooks,external-io -->
- **Bounded awaits**: every `await` against external IO (fetch, subprocess, file watch, db lock waits) MUST have an explicit timeout (`AbortSignal.timeout`, `Promise.race` with `setTimeout`, or wrapper-level kill). Unbounded awaits are a hang vector — the 2026-05-14 Phase 13.1 heartbeat hang ("Phase 2 extract_directives never returns") was an unbounded `await callLocalLLM(...)` against a stalled Ollama daemon. Audit: `context/measurements/2026-05-18-angel-await-audit.md`.
...
<!-- critical: drift-risk=correctness, domains=schema,storage,epoch -->
- **Epoch units are encoded in column names.** `_epoch` (no suffix) = SECONDS (legacy V14-pre-V35 shape). `_epoch_ms` = MILLISECONDS (V35-canonicalized shape, the target). Both conventions coexist in the substrate (audit: `context/measurements/2026-05-18-legacy-epoch-columns-audit.md` lists 24 legacy `_epoch` columns across 16 tables, all storing seconds consistently). Until the full rename phase ships, RESPECT THE NAME: read `_epoch` as seconds, multiply by 1000 before passing to `new Date()`. Read `_epoch_ms` as ms directly. Don't divide `_epoch_ms` by 1000 unless converting to a sec API. New tables MUST use `_epoch_ms` only — never add an `_epoch` column. The 2026-05-17 LSS-debug failure was the agent dividing seconds-as-seconds by 1000 and getting 1970 — that's the failure mode this rule prevents.
