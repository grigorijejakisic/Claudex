...
describe('checkTables', () => {
  ...
  it('reports count of present tables', () => {
    ...
    try {
      ...
      const result = checkTables(db);
      expect(result.status).toBe('fail');
      expect(result.message).toContain('0/24');
    } finally {
      db.close();
    }
  });
  ...
  it('fails when a table has incompatible constraints', () => {
    ...
    try {
      ...
      db.exec(`
        ...
        CREATE TABLE checkpoint_tracking (session_id TEXT PRIMARY KEY, last_checkpoint_epoch_ms INTEGER, thresholds_hit TEXT NOT NULL DEFAULT '[]', observation_count INTEGER NOT NULL DEFAULT 0, post_compact_pending INTEGER NOT NULL DEFAULT 0, updated_at_epoch_ms INTEGER NOT NULL DEFAULT 0);
