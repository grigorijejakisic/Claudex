...
// --- Priority 2.1: Project Curated Context integration ---

describe('assembleFullContext — curated context (P2.1)', () => {
  let db: TestDatabase;
  beforeEach(() => { db = createTestDb(); });
  afterEach(() => { db.close(); });

  it('injects the curated_context section when entries exist', async () => {
    const { writeEntry } = await import('../../core/curated-context.js');
    writeEntry(db, {
      project: 'proj-curated',
      type: 'mental_model',
      content: 'the current theory is X, not Y',
      curator: 'agent',
    });

    const projDir = mkDir('curated-basic');
    const result = assembleFullContext({
      db, project: 'proj-curated', projectDir: projDir, config: makeConfig(),
    });

    expect(result.sources).toContain('curated_context');
    expect(result.content).toContain('## Project Curated Context');
    expect(result.content).toContain('the current theory is X');
  });

  it('includes global curated entries when assembling for any project', async () => {
    const { writeEntry } = await import('../../core/curated-context.js');
    const { GLOBAL_PROJECT_SCOPE } = await import('../../shared/constants.js');
    writeEntry(db, {
      project: GLOBAL_PROJECT_SCOPE,
      type: 'preference',
      content: 'prefer technical accuracy over flattery',
      curator: 'agent',
    });

    const projDir = mkDir('curated-global');
    const result = assembleFullContext({
      db, project: 'any-project', projectDir: projDir, config: makeConfig(),
    });

    expect(result.sources).toContain('curated_context');
    expect(result.content).toContain('Global — Rules & Preferences');
    expect(result.content).toContain('prefer technical accuracy');
  });

  it('does not inject curated_context when no entries exist', () => {
    const projDir = mkDir('curated-empty');
    const result = assembleFullContext({
      db, project: 'empty-proj', projectDir: projDir, config: makeConfig(),
    });
    expect(result.sources).not.toContain('curated_context');
  });

  it('renders curated_context between project and session_continuity', async () => {
    const { writeEntry } = await import('../../core/curated-context.js');
    writeEntry(db, {
      project: 'proj-order',
      type: 'mental_model',
      content: 'order-test theory',
      curator: 'agent',
    });

    const projDir = mkDir('curated-order');
    writeFile(projDir, 'PROJECT_PRIMER.md', 'Test primer');

    const result = assembleFullContext({
      db, project: 'proj-order', projectDir: projDir, config: makeConfig(),
    });

    const projectIdx = result.content.indexOf('## Project');
    const curatedIdx = result.content.indexOf('## Project Curated Context');
    expect(projectIdx).toBeGreaterThanOrEqual(0);
    expect(curatedIdx).toBeGreaterThan(projectIdx);
  });
});
