 ...
 * Composes caller signal with internal timeout signal.
 * Timeout covers body read; rejects responses exceeding MAX_RESPONSE_BYTES.
 * Enforces size limit on actual body text, not just content-length header.
 */
export async function fetchJsonWithTimeout(
  ...
  try {
    ...
    // Read as text to enforce size limit on actual body (not just content-length header)
    ...
    // Non-throwing: no DB access — network error, timeout, or JSON parse failure; caller handles null
    clearTimeout(timeout);
    return null;
  }
}
