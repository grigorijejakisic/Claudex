...
function retrieveContext(
  ...
  try {
    ...
    if (words.length > 0) {
      ...
      const ftsResults = db.prepare(
        `SELECT a.id, a.content, bm25(artifacts_fts) as rank
         FROM artifacts a
         JOIN artifacts_fts fts ON fts.rowid = a.id
         WHERE artifacts_fts MATCH ? AND a.project = ?
         ORDER BY rank LIMIT ?`
      ).all(ftsQuery, project, topK) as Array<{ id: number; content: string; rank: number }>;
  ...
  if (queryEmbedding) {
    try {
      ...
      ).all(project) as Array<{ id: number; content: string; embedding: Buffer }>;

      const scored: Array<{ id: number; content: string; sim: number }> = [];
      for (const c of candidates) {
        const vec = new Float32Array(c.embedding.buffer, c.embedding.byteOffset, c.embedding.byteLength / 4);
        scored.push({ id: c.id, content: c.content, sim: cosineSimilarity(queryEmbedding, vec) });
      }
...
async function generateAnswer(
  ...
  if (questionType === 'temporal-reasoning') {
    typeInstruction = 'Pay attention to dates and time references. Answer with specific temporal information.';
  } else if (questionType === 'knowledge-update') {
    typeInstruction = 'If the user\'s situation changed over time, provide the MOST RECENT information.';
  } else if (questionType === 'multi-session') {
    typeInstruction = 'Combine information from multiple conversations to form your answer.';
...
Use ONLY the provided conversation context. If the answer is not in the context, say "I don't have that information."
${typeInstruction}
...
Answer concisely:`;
  ...
  return ollamaGenerate(ANSWER_MODEL, prompt, 200);
}
...
async function main() {
  ...
  for (const instance of data) {
    ...
    const predicted = await generateAnswer(
      instance.question, instance.question_type, context, isAbstention,
    );
