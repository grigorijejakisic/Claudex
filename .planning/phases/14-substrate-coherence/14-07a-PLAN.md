...
must_haves:
  truths:
    - "The unified `artifact` table is V17's existing `artifact` table — extended in place. Legacy `artifacts` is preserved as a read-only mirror until one milestone post-cutover (per 14-07-CONTEXT.md Locked Decision 5 rollback path). No new top-level table is introduced; V17 becomes canonical."
    ...
    - "FTS5 + vec0 sidecar triggers updated for the unified shape. `artifact_fts_v17` + `vec_artifact_v17` are the canonical search/vector stores after this plan lands. Legacy `artifact_fts` + `vec_artifacts` stay populated as read-only mirrors until cutover."
    - "Migration step is `migrateV36toV37`. v6.6.0 shipped at V36 (hook registration in `bun run setup` references V36). 14-07a increments `PRAGMA user_version` by 1 and adds a row to `schema_versions`. Reverse migration `migrateV37toV36` is present (drops `artifact_id_map`, restores legacy as primary; rollback path only)."
