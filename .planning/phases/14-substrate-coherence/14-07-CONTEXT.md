...
8. **Wave 0 (Foundations) ships first.** Operator-confirmed 2026-05-16 16:21. Contents: (a) auto-commit hooks [SHIPPED 2026-05-16], (b) `/verify` skill, (c) CLAUDE.md verify-before-done rule [SHIPPED 2026-05-16], (d) `sections.ts` split into multiple files. Reason: Wave 0 is the protocol that catches the failure mode that produced the 2026-05-16 v7.0.0 spec errors (writing without verification baseline). Every downstream /auto-orchestrate worker uses `/verify` before claiming done; auto-commit captures their state changes. Sections.ts split eliminates the cross-plan collision risk in Waves 2 and 3.
9. **Experience-tier rewrite (14-07h) keeps cross-project transfer.** Operator-confirmed 2026-05-16 16:21. The original spec said "filter to same-project-only" which would have killed the tier's entire purpose. Corrected: raise relevance bar via combination of (a) per-task-pattern relevance scoring + (b) handle-overlap threshold lift on existing `stageOneHandleOverlap`. Cross-project transfer preserved; out-of-domain noise (OAuth/Expo/bet365 in the claudex-v3 session-start) reduced through better matching, not filter-elimination.
10. **Autonomous one-run /auto-orchestrate authorized (Option B).** Operator-confirmed 2026-05-16 18:13. Scope: one /auto-orchestrate dispatch executes `w0d` → Wave 1 (a → b → c with `--apply --confirm-non-interactive`) → Wave 2 (LINKS-SCHEMA → d/e/f/g; hard-link proposer flag stays OFF) → Wave 3 (h/i/j). Trade-off acknowledged: autonomous cutover means the destructive read-only-flag flip + bulk re-vectorization runs without a mid-flight operator stop. Mitigations in place: auto-commit hooks capture per-step state for rollback diffs; `/verify` protocol runs at each worker's done-claim; benchmark gate at 14-07c will REFUSE cutover on regression (gate failures are halt events, not skip events). Post-/auto-orchestrate operator actions remain: (a) review Wave 1 gate-results file, (b) review 14-07f hard-link UX simulation output, (c) run Wave 3 qualitative disposition test on big-mozzy + claudex-v3, (d) tag v7.0.0 + push (operator-gated). Per `memory/feedback_precommit_binds_metric_not_correctness.md`: this decision shifts operator-stop from mid-run to post-run; the qualitative ship gate is unchanged.
