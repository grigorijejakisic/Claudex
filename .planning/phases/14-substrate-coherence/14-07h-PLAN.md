...
must_haves:
  truths:
    - "Three concerns folded into one plan, all addressing the 2026-05-16 operator audit of session-start coherence (the conversation that produced CONTEXT): (1) MEMORY.md regenerator fix — preserve Lessons index + User Notes across regenerations; (2) lesson `trigger:` frontmatter field used in MEMORY.md output (replace truncated-body with trigger condition + rule); (3) experience-pattern passive injection filtered to project-scope."
    ...
    - "Experience-tier project-scope filter: passive injection (the per-turn `## Past Experience — Relevant Patterns` block) is restricted to in-project patterns. Cross-project patterns remain available via `claudex_search` but are not surfaced passively. Filter is configurable threshold (default: same-project-only). Per CONTEXT in-scope list, this addresses the noise concern from 2026-05-16."
    - "Position-unless-flagged: alternative for cross-project patterns is to surface them with a clear label (`[cross-project]`) instead of filter-out. I lean filter-out because labels still consume budget; filter-out frees budget for higher-signal in-project content. If PM flags this, the alternative is the `[cross-project]` label variant."
  ...
  artifacts:
    ...
    - path: "src/intelligence/experience-tier.ts"
      provides: "Experience tier extended with project-scope filter at passive injection point. Cross-project patterns suppressed from passive surface."
