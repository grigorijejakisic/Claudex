...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/assembly/sections/lessons.ts"
      provides: "Lessons section formatter (H owns this function per WAVE3-COORDINATION). Wave 0 w0d split moved formatProvenPrinciplesSection here from the monolithic sections.ts. H modifies it to render lesson pointers using trigger frontmatter when available; falls back to truncated-body if `trigger:` missing."
