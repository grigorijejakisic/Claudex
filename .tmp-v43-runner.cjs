...
__export(migrations_exports, {
  ...
  detectV2Database: () => detectV2Database,
  initializeSchema: () => initializeSchema,
  migrateFromV2: () => migrateFromV2,
  migrateV14toV15: () => migrateV14toV15,
...
function migrateFromV2(db, v2DbPath) {
  const targetPath = db.name && db.name !== ":memory:" && db.name !== "" ? path2.resolve(db.name) : null;
  const sourcePath = path2.resolve(v2DbPath);
  ...
  try {
    const migrate = db.transaction(() => {
      ...
      db.exec(TELEMETRY_SCHEMA);
      db.exec(TEAM_COORDINATION_SCHEMA);
      const v2HasTable = (tableName) => {
        const row = db.prepare("SELECT 1 FROM v2.sqlite_master WHERE type='table' AND name = ?").get(tableName);
        return row != null;
      };
      const v2HasColumn = (tableName, colName) => {
        const cols = db.pragma(`v2.table_info(${tableName})`);
      ...
      if (v2HasTable("sessions")) {
        const createdCol = v2HasColumn("sessions", "created_at_epoch_ms") ? "created_at_epoch_ms" : v2HasColumn("sessions", "created_at_epoch") ? "created_at_epoch * 1000" : v2HasColumn("sessions", "started_at_epoch") ? "started_at_epoch * 1000" : "unixepoch() * 1000";
        const endedCol = v2HasColumn("sessions", "ended_at_epoch_ms") ? "ended_at_epoch_ms" : v2HasColumn("sessions", "ended_at_epoch") ? "ended_at_epoch * 1000" : "NULL";
      ...
      const v2Tables = db.prepare("SELECT name FROM v2.sqlite_master WHERE type='table' AND name NOT IN ('observations', 'sessions', 'pressure_scores', 'sqlite_sequence')").all();
      ...
      if (v2HasTable("observations")) {
        const rows = db.prepare("SELECT id, files_modified FROM v2.observations WHERE NOT json_valid(files_modified)").all();
...
function detectV2Database() {
  try {
    const home = getClaudexHome();
    const candidates = [
      path2.join(home, "claudex.db"),
      path2.join(home, "db", "claudex.db")
    ];
    for (const candidate of candidates) {
      if (!fs.existsSync(candidate)) continue;
      try {
        const testDb = new import_better_sqlite3.default(candidate, { readonly: true });
        try {
          const row = testDb.prepare("SELECT MAX(version) as version FROM schema_versions").get();
