...
- **13-06 Skill obsolescence — DELETED 2026-05-14 (window accelerated to same day)** — Deletion-action documents authored at `src/skills/auto/starthere-deprecation-notice.md` and `src/skills/auto/endsession-deprecation-notice.md`. Skills lived in `~/.claude/skills/` (outside project CWD). Original gate: one-week deprecation window 2026-05-14 → 2026-05-21. **Window cast away same day** after operator-run end-to-end substrate test passed: planted fact `"violets are green"` in session A retrieved by a fresh session B in the same project via `claudex-recall` (4 calls), agent answered `"Green"` and cited the prior session's failure record. `bun run vesna` 29/29 at 100% held. Skills removed with `rm -rf` on 2026-05-14. Phase 13.1 substrate work (Angel heartbeat hang, MEMORY.md regenerator wiping Lessons index, session_highlights extraction) remains open and documented; deletion proceeded because the rose/violet test demonstrated the substrate carries what the skills used to ritualize.

### Phase 13.1 Open Work (carries forward)

- **Angel heartbeat loop hangs after restart with the new build** — first tick scheduled at +5s (heartbeat.ts:1630) but never observed even after 6+ minutes. `heartbeatTick(ctx)` suspected to await something that doesn't resolve. Sessions-indexer is correct in isolation (verified via `scripts/oneshot.cjs` — 71 chunks indexed from session 523e018e, 5 cursor entries across projects). Workaround: manual `node scripts/oneshot.cjs` invocation periodically. Fix path: localize the hang via inline logging at each `await` boundary in `heartbeatTick`, add defensive indexer trigger to a CC hook so retrieval doesn't depend on a single guardian.
- **`session_highlights` table was missing despite `user_version=33`** — migration runner short-circuits at `version >= TARGET_VERSION`; the missing-table case never self-heals. Created directly via DDL matching `migrateV32toV33` on 2026-05-14. Root cause unknown — possibly an earlier process bumped `user_version` without successfully running V33's CREATE TABLE.
- **MEMORY.md auto-regenerator wipes the Lessons index** — happened 2026-05-14 mid-session; 8 indexed memory pointers replaced with "No lessons captured yet." while the memory files themselves remained on disk. User Notes section survived. Need to either fix the regenerator to scan `memory/*.md` files, or accept User Notes as the durable surface.
- **Defensive indexer hook trigger** — current architecture has the indexer only in Angel. If Angel breaks, substrate stops indexing. Phase 13.1 should add a fallback trigger in session-start or post-tool-use.

### Phase 13.1 Workaround Already Shipped

`scripts/one-shot-reindex.ts` (built to `scripts/oneshot.cjs` via esbuild) — manual driver for `scanAndIndexSessions` against the live DB. Imports the indexer directly from `src/angel/sessions-indexer.ts`, bypassing Angel's main() guard. Run with `node scripts/oneshot.cjs`. Idempotent thanks to the cursor table.
